...
def plot_side_by_side(data_file_path, solutions_file_path, error_file_path, cut_fraction=0.2, zoom_factor=0.5, channel=4):
    # --- 1. Load all data sources ---
    all_data = np.load(data_file_path)
    xyz_coords = all_data[:, 0:3]

    # Channel mapping for labels
    channel_labels = {3: 'Bx', 4: 'By', 5: 'Bz'}
    channel_label = channel_labels.get(channel, f'Channel {channel}')

    ground_truth_colors = all_data[:, channel] 
    prediction_colors = np.load(solutions_file_path)[:, channel-3]
    ...
    fig.colorbar(scatter1, ax=ax1, label=f'{channel_label} Component Value', shrink=0.6)
    ...
    fig.colorbar(scatter2, ax=ax2, label=f'{channel_label} Component Value', shrink=0.6)
...
plot_side_by_side("Samples/HSX_QHS_nsample_12288_nbdry_3072_low_discrepancy.npy", "Evaluation/solutions.npy", "Evaluation/error.npy", cut_fraction=0.0, zoom_factor=0.65, channel=5)